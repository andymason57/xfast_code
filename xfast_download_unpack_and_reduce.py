...
# TODO sort out build up of postgres connections
# TODO find out why region files do not cover all sources
# TODO sort out selecting flare cut off from pipeline processed products leads to no GTIs
# TODO sort out BACKSCAL problem - try other SAS task
# TODO look into small gap routine call - understand - ask lucy.
# TODO put in code to deal with clean/unclean obs/sources - IDL.

#global variable - path to download directory
base_dir = "/mnt/4tbdata/xmm_reduce_results/NOT_course_results"
lrztar_output_dir = "/mnt/4tbdata/xmm_reduce_results/NOT_course_results_tarred"
...
def fetch_all_obsIDs():
    try:
        ...
        fetch_data.execute("SELECT obs_id FROM not_course;")
...
# Then aall aioclient -- need to have a copy of aioclient and lib dir. in download directory
...
def download_data(built_up_list):            
    for str_row in built_up_list:
            try:
                ...
                #open cursor
                cur = conn.cursor()
                cur.execute("INSERT INTO processed_not_course (obs_id) VALUES (%s)", (str_row,))  
                cur.execute("DELETE FROM not_course WHERE obs_id = %s", (str_row,))
                conn.commit()   
            except psycopg2.DatabaseError:
...
#Call Lucy's IDL script to reduce data - need to pass directory!
            ...
            try:
                idl_call = "cd " + base_dir + " && idl -e xmmpipeworkingv26 -args " + " " + base_dir + "/" + str_row + " " + str_row
...
#                      use like lrztar -o compressed_path -L level_no target_dir
            try:
                ...
                print "reached lrzip"
                    ...
                    logger.exception('Some runtime error has occurred in download data def  - with lrztarring files for ObsID: ', str_row)  
            except IOError:
