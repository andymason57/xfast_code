...
base_dir = "/mnt/4tbdata/two"
lrztar_output_dir = "/mnt/4tbdata/two_results"
...
def download_data(built_up_list):
    for str_row in built_up_list:
        try:
            ...
            #open cursor
            cur = conn.cursor()
            cur.execute("INSERT INTO test_cases_two_results (obs_id) VALUES (%s)", (str_row,))
            cur.execute("DELETE FROM test_cases_two WHERE obs_id = %s", (str_row,))
            conn.commit()
            conn.close()
        ...
        try:
            idl_call = "cd " + base_dir + " && idl -e xmmpipeworkingv29_two -args " + " " + base_dir + "/" + str_row + " " + str_row
